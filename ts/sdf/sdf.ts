import { gpu } from "../helpers/gpu-helper";
import { Vec3, Vec4 } from "../helpers/math/vector";
...
export function createSDF(
  ...
  gpu.addFunction(crop);
  const findMinDistance = gpu.createKernel<
    number[][][][],
    {
      faceNormals: Float32Array;
      gridSpaceX: number;
  ...
  >(
    function (grid: number[][][]) {
      const x = this.constants.minx + this.constants.gridSpaceX * this.thread.x;
      const y = this.constants.miny + this.constants.gridSpaceY * this.thread.y;
      const z = this.constants.minz + this.constants.gridSpaceZ * this.thread.z;
      const p: Vec3 = [x, y, z];
        ...
        if (
          ...
          for (let vi = 1; vi < 3; vi++) {
            if (barycentricParams[vi] > largestBarycentricParam) {
              secondLargestBarycentricParamIndex = largestBarycentricParamIndex;
            ...
            } else if (barycentricParams[vi] > secondLargestBarycentricParam) {
      ...
      for (let zIndex = 0; zIndex < gridCountZ; zIndex++) {
        ...
        for (let i = 0; i < indexBuffer.length; i += 3) {
          ...
          if (
            ...
            VEC.dot(VEC.cross(VEC.subtract(q, v2), VEC.subtract(v1, v2)), n) >=
              0
          ) {
        ...
        const gridIndex =
          (xIndex * gridCountY * gridCountZ + yIndex * gridCountZ + zIndex) * 4;
        sdf[gridIndex] = d;

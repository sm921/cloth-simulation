namespace STRING_SIMULATOR {
  ...
  export class Simulator {
    ...
    simulateByEnergyMinimization(): void {
      ...
      const [descentDirection, norm] = this.findDescentDirection();
      if (descentDirection === null || norm === null) return;
      const [stepsize, descentDirectionPerPosition] =
        this.findStepsize(descentDirection);
      if (norm * stepsize < 1e-3) return;
      this.positions.forEach((p, i) => {
        if (!this.isPointFixed(i))
          p.add(descentDirectionPerPosition[i].multiply(stepsize));
      });
    ...
    private findDescentDirection():
      | [MATH_MATRIX.Vector, number]
      | [null, null] {
      const hessianOfEnergy = this.getHessianOfEnergy();
      const gradientOfEnergy = this.getGradientOfEnergy(this.positions);
      let descentDirection = MATH_MATRIX.Solver.cholesky(
        hessianOfEnergy,
        gradientOfEnergy.multiplyNew(-1).elements,
        true
      );
    ...
    private getHessianOfEnergy(): MATH_MATRIX.Matrix {
      const hessianKinetic = this.getHessianOfKineticEnergy();
      const hessianSpring = this.getHessianOfSpringEnergy();
    ...
    private getHessianOfKineticEnergy(): MATH_MATRIX.Matrix {
      const hessian = MATH_MATRIX.Matrix.zero(
        this.positions.length * 3,
        this.positions.length * 3
      );
      for (let i = 0; i < hessian.width; i++)
        // x, y, and z
        for (let j = 0; j < 3; j++)
          hessian.set(
            3 * i + j,
     ...
     * ```
     */
    private getHessianOfSpringEnergy(): MATH_MATRIX.Matrix {
      const hessian = MATH_MATRIX.Matrix.zero(
        this.positions.length * 3,
        this.positions.length * 3
      );
      for (
        ...
        positionIndexI < this.positions.length;
        positionIndexI++
      ) {
        ...
        for (let spring of this.getSpringsConnectedToPosition(positionIndexI)) {
          const positionIndexJ =
            spring.position1Index === positionIndexI
              ...
              : spring.position1Index;
          const [qi, qj] = [
            this.positions[positionIndexI],
            this.positions[positionIndexJ],
          ];

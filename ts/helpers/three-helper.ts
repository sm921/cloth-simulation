namespace THREE_HELPER {
  const pastelColors = [0xf4ccdb, 0xccf5e2, 0xe4c3f4, 0xf4f59b, 0xd3d3f5];

  export function geometry(vertices: number[]): THREE.BufferGeometry {
    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute(
      "position",
      new THREE.BufferAttribute(new Float32Array(vertices), 3)
    );
    return geometry;
  }

  export function line(
    scene: THREE.Scene,
    vertices: number[],
    color?: number
  ): number {
    const _color = color ?? randomPastelColor();
    const points = [];
    for (let i = 0; i < vertices.length; i += 3)
      points.push(
        new THREE.Vector3(vertices[i], vertices[i + 1], vertices[i + 2])
      );
    const geometry = new THREE.BufferGeometry().setFromPoints(points);
    const line = new THREE.Line(
      geometry,
      new THREE.LineBasicMaterial({ color: _color })
    );
    scene.add(line);
    return _color;
  }

  function mesh(
    scene: THREE.Scene,
    vertices: number[],
    Material: new (
      params?: THREE.MeshBasicMaterialParameters
    ) => THREE.Material,
    color?: number
  ): number {
    const _color = color ?? randomPastelColor();
    scene.add(
      new THREE.Mesh(
        geometry(vertices),
        new Material({
          color: _color,
        })
      )
    );
    return _color;
  }

  export function triangle(
    scene: THREE.Scene,
    vertices: number[],
    color?: number
  ): number {
    return mesh(scene, vertices, THREE.MeshBasicMaterial, color);
  }

  export function randomPastelColor() {
    return pastelColors[
      Math.min(
        Math.floor(Math.random() * pastelColors.length),
        pastelColors.length - 1
      )
    ];
  }

  export function sphere(
    scene: THREE.Scene,
    pos: Vec3,
    r = 0.1,
    color?: number
  ) {
    const _color = color ?? randomPastelColor();
    const geometry = new THREE.SphereGeometry(r, 18, 9);
    const material = new THREE.MeshBasicMaterial({
      color: _color,
    });
    const sphere = new THREE.Mesh(geometry, material);
    sphere.position.add(new THREE.Vector3(...pos));
    scene.add(sphere);
    return _color;
  }
}

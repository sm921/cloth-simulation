...
import { arrayOf, range } from "../math/other";
...
export class Multigrid {
  ...
  constructor(points: number[] | Float32Array, depth: number, gridRatio = 12) {
   ...
   */
  interpolate(coarseLevel: number, e_coarse: Vector, e_fine: Vector): void {
    const blockWidth = 12;
      ...
      setIthBlockOfVector(
        ...
        coarseLevel === 1
          ? new Vector([
              ...
              .transpose()
              .kroneckerProduct(I3)
              .multiplyVector(ithBlockOfCoarseResidual, Vector)
  ...
  restrict(coarseLevel: number): void {
    ...
    for (let i = 0; i < r_coarse.height / blockHeight; i++) {
      let sigma = Vector.zero(blockHeight);
      for (const j of mk[i]) {
        if (coarseLevel === 1) {
          const rj = getIthBlockOfVector(j, r_fine, 3);
          sigma.add(
            new Vector([rj._(0), rj._(1), rj._(2), 1])
              .kroneckerProduct(Matrix.identity(3))
              .multiplyVector(rj, Vector)
          );
   ...
   */
  solveBy2LevelMethot(A: Matrix, x: Vector, b: Vector, smoothCount = 2): void {
    for (let i = 0; i < 3; i++) {
      this.updateA(A);
      this.makeA1FullRank();
      Solver.gaussSiedel(A, x, b, smoothCount, undefined);
      const r0 = b.subtractNew(A.multiplyVector(x, Vector));
      const r1 = this.Ut[0].multiplyVector(r0, Vector);
      const e1 = this.A[1].inverseNew()?.multiplyVector(r1, Vector) as Vector;
      const e0 = this.U[0].multiplyVector(e1, Vector);
  ...
  private initA(): void {
    ...
    this.r.forEach((residual, i) => {
      this.A[i] = Matrix.zero(residual.height, residual.height);
    });
  }
  ...
  private initR(positions: Float32Array | number[]): void {
    this.r = this.grids.map((grid, i) =>
      Vector.zero((i === 0 ? 3 : 12) * grid.length)
    );
    this.r[0] = new Vector(positions);
  }

  private initU(points: Vector[]): void {
  ...
  private initUn(points: Vector[]): void {
    for (let level = 0; level < this.grids.length - 1; level++) {
      const fineGrid = this.grids[level];
      const coarseGrid = this.grids[level + 1];
      const blockWidth = this.blockSize;
      ...
      for (
        ...
        const n = 4;
        let nearestPointIndices = range(n);
        let minDistances = arrayOf(Infinity, n);
        for (
          let coarsePointIndex = 0;
          coarsePointIndex < coarseGrid.length;
          coarsePointIndex++
  ...
  private updateA(A: Matrix): void {
    ...
    for (let level = 0; level < this.grids.length - 1; level++) {
      ...
      // const sigma = Matrix.zero(blockSize, blockSize);
      ...
      //         return new Vector([xyz._(0), xyz._(1), xyz._(2), 1]);
      //       });
      //       sigma.add(
      //         xi
...
function buildGrids(
  ...
  const pointsToAddIndices = range(points.length);
...
function getIthBlockOfVector(
  ...
  return new Vector(
    range(numberOfCoordinates).map((coordinate) =>
...
function pointsToVectors(points: number[] | Float32Array): Vector[] {
  const vectors: Vector[] = Array(points.length / 3);
  for (let i = 0; i < vectors.length; i++)
    vectors[i] = new Vector([0, 1, 2].map((xyz) => points[3 * i + xyz]));
  return vectors;
}
/**
...
function block(
  ...
  const block = Matrix.zero(blockWidth, blockHeight);

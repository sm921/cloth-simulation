export class Matrix {
   ...
   * @returns
   */
  columnVector<T extends Matrix>(
    columnIndex: number,
    Vector: new (elements: Float32Array) => T
  ): T {
    const columnVector = new Vector(new Float32Array(this.height));
  ...
  multiplyVector<T extends Matrix>(
    v: T,
    P: new (elements: number[] | Float32Array) => T
  ): T {
    const productAsMatrix = this.multiply(v);
    const product = new P(productAsMatrix.elements);

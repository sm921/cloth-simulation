...
import { qr } from "./matrix-decomposition";
import { hessenbergReduction } from "./matrix-modification";
...
export function eigenvalues(A: Matrix, tolerance = 1e-2): number[] {
  ...
  for (let m = H.height - 1; m > 1; m--) {
    do {
      const shift = wilkinsonShift(H._(m - 1, m - 1), H._(m, m - 1), H._(m, m));
...
export function eigenvectorOf(
  ...
  while (true) {
    x = shiftInv.multiplyVector(x, Vector).normalize();
    if (shift.multiplyVector(x, Vector).squaredNorm() <= 1e-2) return x;
  }
}
...
function wilkinsonShift(
  ...
  return (
    bottomRight + d - Math.sign(d) * Math.sqrt(d * d + bottomLeft * bottomLeft)
  );
}
